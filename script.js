...
(function () {
  ...
  function renderGuestChart(categories) {
    ...
    const padding = { top: 10, right: 10, bottom: 40, left: 30 };
    ...
    categories.forEach((cat, index) => {
      ...
      const labelBaseY = padding.top + chartHeight + 14;
      lines.forEach((ln, lineIdx) => {
        chartCtx.fillText(ln, x + barWidth / 2, labelBaseY + lineIdx * 11);
      });
    });
  }
  ...
  function fetchGuestData() {
    if (!GUEST_API_URL || GUEST_API_URL === "PASTE_YOUR_WEB_APP_URL_HERE") {
      guestStatusEl.textContent =
        "Guest chart not connected yet. Please add your Apps Script Web App URL in script.js.";
      return;
    }
    ...
    fetch(GUEST_API_URL, { cache: "no-store" })
      ...
      .then(data => {
        ...
        if (Array.isArray(data.categories)) {
          ...
          guestStatusEl.textContent =
            "Live breakdown from Google Sheets (updates every 30 seconds).";
        } else {
          guestStatusEl.textContent =
            "Could not find categories in the response. Please check Apps Script.";
        }
      })
      .catch(error => {
...
- Column J: COUNTIF result (number of guests) for that group
  starting from row 2 (row 1 is a header).
...
function doGet(e) {
  ...
  // Labels in Column I, values in Column J (from row 2 down)
